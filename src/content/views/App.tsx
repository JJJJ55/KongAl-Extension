...
import { useIsToken } from '@/hooks/useCheckTokenHook'

function App() {
  const { info, settings, isInit, updateData } = useStoragestore()
  const { isSame } = useIsToken()
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.type === 'GET_NAME') {
      ...
      sendResponse({ lmsUser: altTexts[0] })
    }
  })
  ...
  return (
    <CommonContainer>
      {isSame && settings.siteToken && settings.siteToken!.length === 64 && <Content />}
    </CommonContainer>
  )
}
